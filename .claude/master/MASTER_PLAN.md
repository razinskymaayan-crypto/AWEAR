...
### יחס לשכבת האסטרטגיה (`strategy/` — חידות היסוד)
> `.claude/master/strategy/` בונה את התוכנית העסקית של **מוביל קטגוריה עולמי** (post-raise scale).
> **אין סתירה — יש שני אופקים:** MASTER_PLAN שולט בהחלטות ה-demo של 2–4 השבועות; `strategy/` שולט בתכנון שאחרי הגיוס. **כשהם מתנגשים על משימת ביצוע — ה-demo מנצח.** מסקנת strategy לא משנה scope של demo בלי הכרעת מייסד (FOUNDER_QUESTIONS). סוכני האסטרטגיה (Amancio/Anna/Bernard/Tobi) כותבים מסמכי הכרעה בלבד — לא קוד, לא dispatch לצוותי הביצוע; כרמל מאשרת.
