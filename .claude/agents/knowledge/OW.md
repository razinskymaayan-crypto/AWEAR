...
**מנגנון:** התלבטות כיוון שאי-אפשר להכריע מ-spec נעול → כתוב שאלה ב-`.claude/master/FOUNDER_QUESTIONS.md "## OPEN"` (אופציות + המלצה + loop-stage), קח משימה אחרת (לא להישאר idle). פעם ביום המייסדים מעבירים ל-"## ANSWERED"; directive שעונה = עדיפות עליונה בריצה הבאה (מיד אחרי בילד שבור).
