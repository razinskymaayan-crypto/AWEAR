...
### OW-015 | בדיקה שעוברת ≠ כיסוי — ספור את המשטח לפני שאתה מדווח "תקין"
**מקור:** מייסד, 2026-07-20 — "איך רק עכשיו גילית? זה מובן מאליו." `check-interactions.mjs` נבנה כדי לאמת באג אחד (sheet תקוע), בדק **2 overlays**, החזיר "✓ interactions OK" — בעוד ב-SPA יש **21 פונקציות `open*`**. כיסוי ~10% שדיווח "תקין" הרדים את כולם ליום שלם, והמייסד מצא חלונות תקועים בכל מקום.
**לקח:** בדיקה שנכשלת מזיקה פחות מבדיקה ש**עוברת** ומייצרת ביטחון שקרי. תיקון מאמת את *המקרה*; רק ספירת-משטח מאמתת את *המחלקה*. "יש בדיקה" ≠ "המחלקה מכוסה" (אחות של [[OW-005]]).
**מנגנון:** לפני שמדווחים "עובר/תקין" על בדיקה שמייצגת מחלקת-באגים: (1) **ספור את המשטח** בפקודה אחת (`grep -c "function open"`, מספר ה-endpoints, מספר המסכים) ורשום **N מכוסים מתוך M קיימים** בדיווח; (2) אם הכיסוי חלקי — אמור זאת מפורשות ("מכסה 2/21"), לעולם לא "✓ תקין" סתם; (3) פער כיסוי = פריט backlog, לא הערת-שוליים. כלי ה-sweep: `scripts/ux-audit.mjs` (מונה overlays/ניגודיות/חפיפות ומדווח כמה נוהגו בפועל).
