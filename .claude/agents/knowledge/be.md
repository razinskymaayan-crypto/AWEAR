...
## BE-TAG-INTEGRITY — cross-file id references need an orphan check + cache reload (2026-07-04, oren)
`posts.json.items_tagged` referenced `prod_015`-format ids that never existed in `products.json` (`prod_ss_NNN`/`prod_jk_NNN`/... format). Result: 100% of feed item-pills opened a broken sheet — `productMap[pid]` was `{}`, and the SPA fallback `name: prod.name || pid` leaked the raw id ("prod_015") as the garment name, category='top', price=0, no image. Two traps: (1) The founder's DoD grep was a 3-id spot-check (`prod_015|prod_032|prod_041`) but the real blast radius was ALL 40 posts — always verify orphans programmatically (`every tag ∈ set(product ids)`), not by the sample grep alone. (2) `--reload` watches only `.py`; `_posts_cache`/`_products_cache` are read from JSON once at startup, so editing `posts.json` did NOT update what `/api/posts` served to mobile — the server kept serving broken ids until a real restart (`pkill uvicorn` → relaunch). A data fix to a cached JSON is not live until the cache reloads — verify via `curl /api/<endpoint>`, not just the file.
**Fix pattern:** (a) build an honest, caption-matched remap (real garments per post vibe), preserve array-of-strings shape + original length; (b) prove 0 orphans against products.json AND against the live `/api/posts` after restart; (c) harden the SPA fallback to fail loud — unresolved tag returns `null` + `console.warn`, then `.filter(Boolean)`, so a raw id can never render as a name (index.html ~4516). Since both web (loads posts.json directly) and mobile (`/api/posts` → `_posts_cache`) read the same array, one data fix covers all layers — but the server restart is the third, easily-forgotten layer.

## BE-DIAG-LIVENESS — a "key configured" flag is NOT a "key works" signal (2026-06-29, sam)
A boolean like `key_configured = bool(os.getenv("ANTHROPIC_API_KEY"))` only proves a STRING is set — it cannot tell "key present but invalid/expired/SDK-mismatch" from "key works". When a paid LLM call silently falls back to demo, that ambiguity is the whole bug. Give the diagnostic an OPT-IN liveness probe (`?probe=1`) that makes ONE minimal real call (`messages.create`, cheap model, `max_tokens=5`) and returns a definitive `key_valid` + a BOUNDED `probe_error` enum — never `str(e)` or key material in the response (log raw with `exc_info=True`). Gate the paid path with a SEPARATE tighter rate-limit bucket (e.g. 3/min via `check_rate_limit_window`) so it can't be hammered, and keep the default (no-probe) path free/no-call. Use `messages.create` for the probe (exists on every SDK version) so a probe failure means credentials/reachability, not SDK shape.
Related: structured-output `client.messages.parse(...)` is GA on current SDKs but was beta-only on older ones — wrap the call and on `AttributeError` retry `client.beta.messages.parse(betas=["structured-outputs-2025-11-13"], ...)` so a lagging deploy box runs LIVE instead of silently dropping to demo. Factor the exception->enum mapping into one helper reused by both the live endpoint and the probe.
