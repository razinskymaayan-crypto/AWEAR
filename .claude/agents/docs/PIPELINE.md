...
- **Telegram**: הכל דרך `scripts/tglib.py` (canonical: chunking 4096, retries, 429, לוג כשלים ב-`.claude/telegram_failures.log`). `tg.sh` הוא wrapper דק — לא להוסיף curl ישיר
