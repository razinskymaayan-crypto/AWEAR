...
```mermaid
flowchart LR
    IC["IC כותב proposal<br/>agents/plans/"] --> Peer["Peer review מסוכן IC אחר<br/>(לא ראש הצוות — שווה-לשווה)"]
    Peer -->|תיקונים נדרשים| IC
    Peer -->|עבר| Head["ראש הצוות מושך,<br/>בודק ביקורתית"]
    Head -->|נדחה / לתיקון| IC
    Head -->|מאושר| JeffReview["ג'ף קורא + מתייעץ<br/>עם שאר ראשי הצוותים"]
    JeffReview -->|לא רלוונטי כרגע| Head
    JeffReview -->|אושר| Live["שילוב בפועל ל-<br/>static/index.html / app.py"]
    Live --> Log["activity_log.md מתעדכן<br/>עם הוכחה (commit hash)"]
```

**קריאה לכולם, כתיבה רק לצוות שלך:** כל סוכן כותב proposals ב-`agents/plans/` וקורא חופשית בכל `agents/`. נאכף ע"י tools מוגבלים בכל subagent רשום (`.claude/agents/*.md`) + ביקורת ג'ף לפני מיזוג.
...
- `agents/plans/` — כל הסוכנים כותבים לכאן proposals, הכרעות, תוכניות. קריאה וכתיבה לכולם.
- `agents/logs/` — תיעוד היסטורי: activity_log, sprints, board syncs. לא עורכים — רק מוסיפים.
- `agents/docs/` — מסמכי מדיניות קנוניים (המדריך הזה, daily_model, org). שינוי דורש אישור ג'ף.
- `agents/data/` — קבצי JSON של סטטוס חי (live_status, meeting_status, agent_notes).
- שילוב בפועל לקוד החי קורה **רק** אחרי שג'ף בדק ואישר — אף סוכן לא commit-ית ישירות ל-main בלי לעבור את התהליך.
...
- סטטוס (עובד כרגע / לא פעיל) לפי `agents/data/live_status.json` — ג'ף מעדכן את הקובץ הזה **בזמן שיגור** משימה ומנקה אותו כשהתוצאה חוזרת (ראה כלל #16).
