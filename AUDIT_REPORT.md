...
**Combined P4 review (phases 5/6/8) findings & resolution:** 1 BLOCKER — DECISIONS row 13 falsely implied awear.db is fully regenerable (user-persisted rows live ONLY there; wording could invite data loss) → reworded with explicit warning. Row 3's "generates tokens.css" claim false (no generator script exists — hand-sync; fixed here AND in design-tokens.md, a stale claim inherited from the old CLAUDE.md). STATE.md wiring clarified as engine-only in autonomous lanes (deliberate — 6 parallel lanes would conflict; memory.md updated). Row 4 scoped to code lanes (retrospective/telegram-poll push docs directly). P3 jargon glossed in effort.md. 13/15 DECISIONS rows verified true by reviewer grep; phases 6+8 clean passes.
