...
## Mark lane — last run (2026-07-21, run 7)
- **Task**: Marketplace contrast P0s + text truncation. 4 invisible-text bugs (`--accent3` buttons: `var(--fg)` = near-black on dark-brown bg, 1.52:1 → `var(--card,#fff)` = 12.65:1 AAA). 9 missing overflow controls in `.mp-item-*` and `.pc-feat-*` cards. Commit da5502f. Gabbana 8/10 PASS.
- **Next run note**: `.mp-item-shop-btn { color: #fff }` (line 1667 app.css) is a pre-existing DS-004 P1 — fix in next run touching mp.
- **Remaining**: INBOX UX bug-hunt items 1 (other screens, non-mp/collections), 2 (stuck sheets/modals). Item 3 (contrast) now DONE.
