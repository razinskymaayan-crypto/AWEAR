...
## Mark lane — last run (2026-08-09, run 71)
- **Task**: Fix look-sheet thumbnail fallback (DS-016) — all 3 demo posts lacked image_url, causing random loremflickr images in look-sheets.
- **Done**: static/app.js (added image_url to 10 items across demo_post_1/2/3, matching _SCAN_DEMO_OUTFITS URLs). node --check + check-render green.
- **Next**: Further gabbana-flagged polish; NEEDS_DECISION #3 (token peg) founder-only.
- **Prior runs**: run 70 — store tab UX + --warning WCAG fix 0f77b14; run 69 — profile look-tap 3-tier buy sheet a554415; run 68 — DS-004 light-theme scope 843722d.
