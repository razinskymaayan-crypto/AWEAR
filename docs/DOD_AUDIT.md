...
| COMMERCE-1 | Skimlinks affiliate deep-link + SubID poster attribution | ✅ VERIFIED | `a6e799f` | `app.py:331` — `SKIMLINKS_ID = "307075X1795350"` (live publisher id); `app.py:342` — `CREATOR_CREDIT_PCT = 0.05` (5%, locked); `app.py:398-408` — `affiliate_url()` wraps any URL in Skimlinks deep-link + `xcust` SubID (`poster_id:post_id`); `app.py:412-415` — `build_buy_options()` uses `affiliate_url()`; `app.py:4928` — creator token credit calculation. `docs/COMMERCE_PLAN.md` added (founder-locked commerce model doc). `grep -n "affiliate_url\|SKIMLINKS_ID\|CREATOR_CREDIT_PCT" app.py` → 8 hits confirmed. |
