...
- **עדכון 2026-07-24:** wardrobe match score backend ✅ **shipped** (commit `9cc466c` by steve lane) — `GET /api/products/{id}/match` מחזיר `match_pct` (0–95), `reason`, `matching_items` מה-closet של המשתמש ב-SQLite; BE-006 `user_key`, rate-limited 30/min, 4 pytests הרמטיים (141/141). ה-UI (Slide 2 + Demo beat 5) עדיין client-side via `calcCompatScore()` — SPA wiring ל-endpoint הוא P1 למארק. DoD Audit #13 נוסף (12/13 מאומתים; 1 backend-only).
