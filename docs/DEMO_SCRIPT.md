...
> 💡 *MATCH-MATRIX ✅ **shipped** commit `5a24c0b` (2026-08-07): Bags and dresses now count in wardrobe match scoring — previously they were silently excluded from the `_COMPLEMENTS` table. If the demo account has scanned a bag or dress, those items now raise the match % shown on the ring. No UI change; the number is simply more accurate.*
