...
> 💡 *LIGHT-MODE-POLISH ✅ **shipped** commits `3b58a1e` + `e51c915` (2026-08-14, mark run-91): 8 light-mode fixes across home greeting gradient, wallet pending text (AA-compliant), hero blob, banner, Withdraw button, story rings, sell button, and outfit CTA. The app is presentation-clean in **both dark and light mode** — if the investor's iPhone or venue lighting leads to a theme switch, the premium look holds in either direction.*
...
> 💡 *STYLIST-DISTINCT-TIPS ✅ **shipped** commit `d790d8e` (2026-08-14, steve run-51): The AI Stylist fallback now returns **3 genuinely distinct outfit tips per occasion** — each tip is uniquely worded (not the same suggestion repeated). The "3 looks, 3 distinct styles" demo story now holds up if the investor cycles occasions in the room. Demo tip: if they ask "does it just repeat itself?" — say: "Three looks, three distinct styles, from the same wardrobe."*

> 💡 *OUTFIT-ANCHOR-ITEM ✅ **shipped** commit `ef26541` (2026-08-14, steve): `/api/outfit/generate` now accepts an `anchor_item` parameter; the tapped product is always featured in every generated look. Demo extension (from beat-5): after the WOW item sheet, if an investor asks "can she build an outfit around *this* piece?" — say: "Every look Abigail generates is anchored to the item she tapped — it's always in the look." The first tip names the anchor item explicitly ("Built around the [item] — …").*
...
> 💡 *FOR-YOU-POSTS-RANK ✅ **fully shipped** — backend `9ffb9b2` (2026-08-13, sam lane) + UI wired by mark run-88 commit `f8d44f7` (2026-08-14): `GET /api/posts?sort_by=match&viewer_id=<id>` ranks feed posts by closet-match score; the For You fetch now sends `?sort_by=match&viewer_id=`. During beat-4: tap the **For You** tab and say: "The entire For You feed is ordered by what fits her style — every post at the top is there because at least one tagged item scores high against her closet." The ranking is live end-to-end.*
...
> 💡 *LOOK-SHEET-PRODUCT-URLS ✅ **shipped** commit `45edbdc` (2026-08-14, mark run-89): `product_url` and `search_query` are now wired from the API post data into every look-sheet item row. Every **"Buy This Item"** button in the look detail sheet opens the exact product page — no placeholder or Google-fallback links during the demo. Beat-5 shoppability is fully end-to-end.*

> 💡 *LOOK-SHEET-EDITORIAL-POLISH ✅ **shipped** commit `af93d52` (2026-08-14, mark run-90): The look-sheet received a full editorial pass — hero cover image (full look photo), 76px item thumbnails, tokenized store-row styling, DS-004 fallback corrected. The look detail is now investor-presentation quality: visually clean, proportional, and consistent with the feed card language. No talking-point change; the sheet simply looks polished.*
...
> 💡 *DEMO-SIMULATE-PURCHASE ✅ **shipped** commit `caf9204` (steve run-53, 2026-08-18): `POST /api/demo/simulate-purchase` lets you **trigger a live purchase mid-beat-7** without real Skimlinks credentials — resolves a random post → poster_id → inserts a pending credit (~$2, 8% affiliate × 40% creator share). **Two beat-7 strategies — pick one:**  (a) **Seeded balance (safe):** run `seed-wallet` in pre-flight → open Wallet → point at $21.35 confirmed + $10.40 pending and say "This is what a week of creator earnings looks like." (b) **Live trigger (dramatic):** skip seed-wallet → open Wallet at $0 → tap a Buy button on a feed post → call simulate-purchase → watch the pending credit appear in real-time and say "Watch — that purchase just landed in her wallet. Append-only ledger, real backend, zero Skimlinks credentials needed for the demo." Strategy (b) is higher-risk but more convincing — use it if you're confident in Wi-Fi/connectivity.*
